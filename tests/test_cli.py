...
def test_chat_cmd_streams_no_chunk_turn_uses_buffered_reply():
    class _NoChunkAgent:
        def __init__(self):
            self.hooks = HookBus()
            self.config = Config()
    ...
    assert "stale previous reply" not in "".join(plain_outputs)
    assert saved == [("sess-1", "hello", "buffered reply")]


def test_chat_cmd_stream_no_chunk_does_not_reuse_stale_assistant_history():
    class _NoChunkAgent:
        def __init__(self):
            self.hooks = HookBus()
            self.config = Config()
